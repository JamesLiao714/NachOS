...
Scheduler::ReadyToRun (Thread *thread)
{
	...
	case SJF:
	    ...
	    cout<<" Thread's name: "<<thread->getName()<< " BurstTime: -> "<<thread->getBurstTime()<<endl;  //印出thread 的名稱和thread SJF的值 
	    break;
	case SRTF:
	   //readyList->Insert(thread);
	   cout <<"Thread name：" <<thread->getid()<<"      BurstTime :"<<thread->getburst()<<"\n"; 
	default:
	     readyList->Insert(thread); 
	     break;
    }
...
Scheduler::Run (Thread *nextThread, bool finishing)
{
	...
	case SJF:
	  cout << "current Thread BurstTime: " <<oldThread->getBurstTime()<<"next Thread BurstTime-> "<<nextThread->getBurstTime()<<endl;  
	    break;
	case SRTF:
	   oldThread->burst -= 10;    //主要負責存放計算新thread的burst time每次burst time為10
    	   cout << "目前Thread 的BurstTime 值" <<oldThread->getid() << " 下一個將要執行Thread 的BurstTime 值"<<nextThread->getid()<<endl; 
	default:
	     cout <<"running............."<<endl;
	     break;
    }
